...
@RestController
@RequestMapping(value = "/api/v1/routine-schedules", produces = MediaType.APPLICATION_JSON_VALUE)
...
public class RoutineScheduleController {
    ...
    /**
     * 루틴 스케줄 삭제 (이 날만 스킵) API
     * @param scheduleId 삭제할 스케줄 ID
     * @param userDetails 로그인된 사용자 정보
     * @return
     */
    @Operation(
        summary = "루틴 스케줄 삭제 (이 날만 스킵)",
        description = "특정 날짜의 루틴 스케줄을 삭제합니다.<br><br>" +
            "<strong>사용 시나리오:</strong><br>" +
            "• 반복 루틴에서 특정 날짜만 제외하고 싶을 때<br>" +
            "• 예: 매주 월,수,금 운동하는데 이번 주 금요일만 스킵<br>" +
            "• 루틴 전체를 삭제하는 것이 아닌, 해당 날짜 스케줄만 제거<br><br>" +
            "*실제 DB에 반영되지 않음*"
    )
    @DeleteMapping("/{scheduleId}")
    public ResponseEntity<CommonApiResponse<Void>> deleteRoutineSchedule(
        ...
        // 3. 스케줄 삭제 (soft delete 권장)
        // 4. 삭제된 스케줄에 대한 포인트 처리 (이미 완료된 경우)
